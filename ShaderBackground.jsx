// ShaderBackground — 21st.dev Shader Builder ("Mesh drift"), adapted.
// Palette retuned to MG Store: deep navy, blue, green.
...
const NAVY_DEEP = [0.0196, 0.0549, 0.1490];
const NAVY = [0.0431, 0.1216, 0.3725];
const BLUE = [0.0863, 0.2118, 0.5882];
const GREEN = [0.0588, 0.4784, 0.1333];

const UNIFORMS = {
  colors: [NAVY_DEEP, NAVY, BLUE, NAVY_DEEP, GREEN, GREEN, GREEN, GREEN],
  colorCount: 5,
  scale: 1.16,
  intensity: 0.34,
  paramA: 0.5,
...
function ShaderBackground({ className, style }) {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const cw = canvas.clientWidth || bounds.width || 1;
      const ch = canvas.clientHeight || bounds.height || 1;
      const rawWidth = Math.max(1, Math.round(cw * dpr));
      const rawHeight = Math.max(1, Math.round(ch * dpr));
  ...
  return (
    <canvas
      ref={canvasRef}
      className={className}
      style={Object.assign({ display: "block", width: "100%", height: "100%" }, style)}
    />
  );
}
